...
function createOptionsUsageString(options, required, threshold, noColor) {
  ...
  if (total < threshold) {
    ...
    options.arguments.forEach(function (option) {
      var argumentString = '-' + option.shortId + ' ' + option.name;
      if (option.many) {
        argumentString += '...';
      }
...
function createOperandsUsageString(operands, threshold, noColor) {
  ...
  Object.keys(operands).forEach(function (id) {
    ...
    var argumentString = option.name;
    if (option.many) {
      argumentString += '...';
    }
...
function createLongString(option) {
  ...
  if (option.type) {
    longString += ' ' + option.name;
    if (option.default) {
      longString += ':' + option.default;
    }
    if (option.many) {
      longString += '...';
    }
...
function createOperandString(option) {
  ...
  operandString += ' ' + option.name;
  if (option.default) {
    operandString += ':' + option.default;
  }
  if (option.many) {
    operandString += '...';
  }
